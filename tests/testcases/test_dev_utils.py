...
class TestDevUtils(unittest.TestCase):
    ...
    def test_should_map_event_as_btn(self):
        ...
        self.assertTrue(do(device, new_event(EV_ABS, ABS_HAT0X, 1), mapping))
        self.assertTrue(do(device, new_event(EV_ABS, ABS_HAT0X, -1), mapping))

        """Mouse movements"""

        self.assertTrue(do(device, new_event(EV_REL, REL_WHEEL, 1), mapping))
        self.assertTrue(do(device, new_event(EV_REL, REL_WHEEL, -1), mapping))
        self.assertTrue(do(device, new_event(EV_REL, REL_HWHEEL, 1), mapping))
        self.assertTrue(do(device, new_event(EV_REL, REL_HWHEEL, -1), mapping))
        self.assertFalse(do(device, new_event(EV_REL, REL_X, -1), mapping))

        """regular keys and buttons"""

        self.assertTrue(do(device, new_event(EV_KEY, KEY_A, 1), mapping))
        self.assertTrue(do(device, new_event(EV_ABS, ABS_HAT0X, -1), mapping))

        """mousepad events"""

        self.assertFalse(do(device, new_event(EV_ABS, ecodes.ABS_MT_SLOT, 1), mapping))
        self.assertFalse(do(device, new_event(EV_ABS, ecodes.ABS_MT_TOOL_Y, 1), mapping))
        self.assertFalse(do(device, new_event(EV_ABS, ecodes.ABS_MT_POSITION_X, 1), mapping))

        """joysticks"""

        self.assertFalse(do(device, new_event(EV_ABS, ecodes.ABS_RX, 1234), mapping))
        self.assertFalse(do(device, new_event(EV_ABS, ecodes.ABS_Y, -1), mapping))
